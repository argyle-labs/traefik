//! traefik service backend — Traefik reverse proxy / ingress.
//!
//! Implements `ServiceBackend` so the generic `service.*` tools
//! (deploy/backup/restore/configure/status/connect/sync) drive traefik. No
...
/// traefik backend. Holds only the provider name; per-instance endpoint/creds
...
impl ServiceBackend for TraefikBackend {
    fn provider(&self) -> &str { self.provider }

    /// Runtimes traefik can be placed on. `service.deploy` hands the
    ...
    /// In-workload paths holding config/data. This is ALL traefik declares for
    ...
    {
        // TODO: describe the traefik workload (image/template, ports, mounts,
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("traefik.workload_spec")) })
    }

    fn configure<'a>(&'a self, _ep: &'a Endpoint, _config: &'a str)
        -> BoxFuture<'a, Result<(), ServiceError>>
    {
        // TODO: apply traefik-specific config idempotently.
        Box::pin(async move { Err(ServiceError::unimplemented("traefik.configure")) })
    }
        ...
        -> BoxFuture<'a, Result<ServiceStatus, ServiceError>>
    {
        ...
        Box::pin(async move { Err(ServiceError::unimplemented("traefik.status")) })
    }
}
...
mod tests {
    ...
    fn declares_provider() {
        let b = TraefikBackend::new("traefik");
        assert_eq!(b.provider(), "traefik");
    }
}
